document.addEventListener("DOMContentLoaded", function() {
  ...
  var html = document.querySelector('html'),
    ...
    workImage = document.querySelector(".work-head__image img");
  ...
  if (workImage) {
    ...
    img.src = workImage.getAttribute("data-src") || workImage.src;
    img.id = "work-image-lightense"
    img.hidden = true;
    document.body.appendChild(img);

    workImage.classList.add("lightense-target");
    workImage.addEventListener("click", () => document.querySelector("#work-image-lightense").click());
  }

  if (lightense || workImage) {
    Lightense(".page img:not(.no-lightense), .post img:not(.no-lightense), .gallery__image img:not(.no-lightense), #work-image-lightense", {
      padding: 60,
      offset: 30,
      beforeShow() {
